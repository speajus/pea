import { createPrimitiveProxy, newProxy, proxyKey, wrapPrim } from "./newProxy";
...
class Context<TRegistry extends RegistryType = Registry> {
  ...
  register(service: PeaKey<TRegistry>, ..._args: any[]): this {
    ...
    const dependencies = _args.reduce((set, arg) => {
      if (isObjectish(arg) && (arg as any)[proxyKey]) {
        set.add((arg as any)[proxyKey] as CKey);
      }
      return set;
    }, new Set<CKey>());
  ...
  private invalidate(key: CKey, seen = new Set<CKey>()) {
  ...
  resolve<T extends (Constructor & Service) | Fn | keyof TRegistry>(
    ...
    if (!this.has(key)) {
      ...
      if (service == null || (isSymbol(service) && _args.length === 0)) {
        throw new PeaError(
          `service '${String(service)}' not registered, and can not be resolved.`,
        );
      }
