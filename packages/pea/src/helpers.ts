...
function get<T, TKey extends string>(obj: T, key: TKey, defaultValue?: PathOf<T, TKey> | undefined): PathOf<T, TKey> {
...
export function pathOf<T extends PeaKey<TRegistry>, TPath extends string, TRegistry extends RegistryType = Registry>(
  service: T,
  path: TPath,
  defaultValue?: PathOf<ValueOf<TRegistry, T>, TPath> | undefined,
) {
  return (ctx = pea(service)) => get(ctx as ValueOf<TRegistry, T>, path, defaultValue);
}
