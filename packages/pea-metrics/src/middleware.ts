...
export const middleware = (ctx = context) => async function middleware$inner(_req: any, res: Response) {
...
export async function apply(v: Express, ctx = context) {
    ...
    v.listen(config.port, config.host, () => {
        console.log(`@speajus/pea-metrics is running on 'http://${host}:${config.port}${path}'`)
    })
}