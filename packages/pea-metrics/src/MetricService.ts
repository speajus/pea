...
export class MetricService {
    constructor(private config = pea(MetricsConfig),
    ...
    withMetric(v: ServiceDescriptor<any, any>) {
        ...
        switch (name) {
            ...
            case '':
            case '<anonymous>':
                console.warn(`can not monitor anonmous pea's please use peaKey, symbol with a description, or name the pea`)
                return;
        }
    ...
    newMetric(oname: string) {
        ...
        if (this.config.metricType === 'summary') {
            return new this.promClient.Summary({
                name,
                help: 'duration summary of method invocation labeled with: ' + labelNames.join(', '),
                labelNames,
        ...
        if (this.config.metricType === 'histogram') {
            return new this.promClient.Histogram({
                name,
                help: 'duration histogram of method invocation labeled with: ' + labelNames.join(', '),
                labelNames,
